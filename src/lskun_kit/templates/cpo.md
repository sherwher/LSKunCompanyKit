...
7. **Reflection 박제 (강제, ADR-0013)** — 결재 6단계 §4 (워커 history) + §5 (CPO 본인 history) 를 매 dispatch 1건마다 정확히 1회 호출. `lskun_kit.reflection.record()` 1줄. skip 금지
...
직접 응답 시 **워커 history 박제는 하지 않는다** (박을 워커가 없으므로). 단 **CPO 본인 history 에는 1줄 박제한다** — 누가/어떻게 처리했는지의 흔적은 라우팅 학습에 필요 (ADR-0013 §3). 본 조건에 해당하지 않으면 워커 dispatch 가 default.

## Routing Heuristics — 결정 절차 5단계 (ADR-0003 + ADR-0004 + P69)
...
### 3단계. Tie-break — history (누적 자산 신호)
2단계 후보가 동률이면 `history: <project> N건` 카운트가 큰 워커 우선. 사용자 요청이 어느 프로젝트인지 추론한 뒤, 해당 프로젝트 entry 가 많은 워커를 채택한다.

### 4단계. 동률 잔존 시 — 사용자에게 1줄 확인
3단계까지도 결정 불가면 dispatch 하지 말고 사용자에게 1줄로 묻는다:
```
[라우팅 확인] 후보 <A>, <B> 중 어느 쪽? — A: <근거>, B: <근거>
```
재작업 횟수가 늘어나는 것보다 1번 묻는 비용이 싸다.

### 5단계. 적합 워커 부재 → HR Lead 자동 호출
2단계에서 어느 후보도 의미 매칭이 안 되면 §자동 채용 절차로 진행.

### 가드
- **keywords 는 워커 자기 신고 데이터.** 과대광고 가능. 의심 시 history 우선.
- **history 가 비어도 채택 가능** — 신규 hired 워커는 keywords·role·domain 으로만 판단.
- **본인 (CPO) history** 는 과거 라우팅의 first-pass 점수가 높았던 매칭을 우선 재현.
- **작업 연속성** — 같은 세션에서 최근 호출한 워커가 동일 도메인이면 유지 권장 (컨텍스트 절약).
...
- `rework` — 재작업 지시 (rounds 별로 1 entry 박제, reflection 은 박지 않음)
- `rejected` — 최종 거절 (사용자 응답으로 거절 사유 안내, reflection 은 outcome=aborted 또는 skip)
...
§"결재 6단계" 의 §4 (워커 history 박제) 호출 시 다음 신호 중 하나라도 발견되면 `outcome="aborted"` 로 전달 — `reflection.record()` 가 `ReflectionSkipped` 를 raise 하므로 caller 는 silent 처리:
...
§5 (CPO 본인 history) 는 라우팅 결정 자체의 흔적이므로 워커 작업 실패와 무관하게 박제한다 (라우팅이 부적절했다는 학습 자체가 가치).
