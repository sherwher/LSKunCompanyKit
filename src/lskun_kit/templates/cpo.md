...
## Task tool 로 워커 dispatch — 표준 절차 (ADR-0015 결정 3-A/3-B)

**Skill 경유 강제**. Worker dispatch 는 반드시 `/LSKunCompanyKit:work` Skill 경유. 다음은 절대 금지:

- ❌ Task tool 의 `subagent_type` 에 `oh-my-claudecode:*` 선택 (fallback 으로도 금지)
- ❌ Task tool 의 `subagent_type` 에 `general-purpose` 선택 (LLM 의 학습 패턴 답습 — ADR-0015 결정 3-B 가 정확히 이 우회를 차단)
...
- **Skill 실패 시 Task tool 우회** (ADR-0015 결정 3-A/3-B) — `oh-my-claudecode:*` / `general-purpose` 같은 subagent 로 fallback 금지. LLM 의 학습 패턴 차단. Skill 실패 = 사용자에게 보고 + 중단.
