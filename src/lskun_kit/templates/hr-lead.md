...
6. **JD body 작성 (ADR-0011 + ADR-0014)**
   ...
   - 출력: 다음 3 섹션을 포함한 markdown string (분량 **300~800자** 권장 — ADR-0024, 도메인 전문성이 실제로 박히는 분량):
     ...
     ```
   - **JD 자가 점검 (P132)** — 파일 박제 직전 1회, 아래 5문항을 스스로 확인하고 하나라도 "아니오" 면 JD 를 고쳐 쓴다. 워커는 채용 시 완성형이라 (ADR-0014) 이 시점의 품질이 전부다. 점검은 HR Lead 의 판단이며 plugin core 는 JD 를 검증하지 않는다 (ADR-0011 — JD schema · 검증 코드 금지):
     1. **도메인 특정성** — 회사 이름과 domain 을 지워도 어느 회사 JD 인지 구별되는가? 어느 회사에나 붙는 문장 ("고품질 코드를 작성한다", "best practice 를 따른다") 뿐이면 다시 쓴다.
     2. **함정의 구체성** — 도메인 함정 · 안티패턴이 "보안에 유의" 수준이 아니라, 무엇을 · 어디서 · 왜 피하는지 적혀 있는가? (예: "PHI 를 애플리케이션 로그에 남기지 않는다 — 마스킹은 로거 계층에서")
     3. **한계 명시** — 이 워커가 **하지 않는 것** (다른 role 의 영역) 이 1줄 이상 있는가? 경계가 없으면 CPO 라우팅이 흐려진다.
     4. **keywords 정합** — `keywords:` 가 직함의 동의어가 아니라 **사용자가 실제로 쓸 요청 표현** 인가? JD 본문이 뒷받침하지 않는 keyword (과대광고) 는 뺀다.
     5. **정적 서술** — "경험을 쌓으며", "점차 개선한다" 같은 성장 · 학습 서사가 없는가? (ADR-0014)
