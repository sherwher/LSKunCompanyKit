...
def decide_target(
    ...
    - ``requested_worker`` 가 비어 있지 않으면 → direct (CPO 경유 안 함).
      hired 부재 시 dispatch 후 ``WorkerNotFoundError`` 가 caller 에 raise 됨.
    - 비어 있으면 → CPO 라우팅 시도
      - CPO 가 hired 되어 있지 않으면 → ``missing-cpo`` (사용자에게 init 안내)

    ADR-0019 (2026-05-27) — Archive 메커니즘 완전 폐기. 옛 archived 가드
    (결정 7-E) 제거. hired 부재 = 단순 ``WorkerNotFoundError``.
    """
...
def build_cpo_routing_context(
    ...
    parts.extend(
        [
            ...
            "",
            "## 응답 형식",
            "적합 워커 선정 후 — Delegation Gate 판정 (ADR-0025):",
            "dispatch 는 다음 3조건 중 하나 이상 충족 시에만.",
            "  ① 컨텍스트 보호 (대량 노이즈 탐색·조사 격리)",
            "  ② 병렬 탐색 (독립 read 서브태스크 2개 이상 동시 진행)",
            "  ③ 독립 검증 (산출물을 clean context 에서 검증)",
            "",
            "게이트 충족 — Task tool 로 dispatch (read-only 기여, 쓰기는 CPO):",
            "```",
            "추천 워커: <worker> (<role>, domain=<domain>, model=<미지정(상속)|frontmatter>)",
            "근거: <한 줄> + 게이트 조건: <①|②|③>",
            "행동: dispatch tool 호출 (subagent_type=\"LSKunCompanyKit:worker\", ADR-0026 —",
            "      쓰기·하위 dispatch 도구 없음) → 워커 JD + Handoff Brief + 사용자 요청 주입",
            "```",
            "",
            "게이트 미충족 — 빙의(embody, 기본 경로): CPO 가 워커 JD body 를 읽어",
            "그 전문가로서 직접 수행. 알림 1줄: \"<display_name>·<role> 관점으로 직접 수행합니다\".",
            "결재 audit 은 동일 박제 — `lskun-audit record ... --embody` (ADR-0027,",
            "reason 에 'embody:' 접두 자동). dispatch 건은 --embody 없이 같은 명령.",
            "증거 게이트 (P127): 빙의 건도 이번 턴의 새 검증 증거 없이 완료 주장 금지 —",
            "증거가 없으면 '미검증' 으로 보고.",
            "",
            "규모 스케일링 (P127) — 게이트 통과 후: ① = 워커 1명 / ② = 독립 서브태스크",
            "수만큼, 상한 4 (브리프 작업 경계가 겹치지 않을 때만) / ③ = verifier 1명.",
            "",
            "워커 보고 상태코드 (P127) — 자가 평가 첫 줄 '상태:' 로 분기:",
            "  DONE → 결재 / DONE_WITH_CONCERNS → 우려 판단 후 결재·rework·에스컬레이션 /",
            "  NEEDS_CONTEXT → 브리프 보강 후 재dispatch / BLOCKED → 원인 해소 또는 사용자 보고.",
            "",
            "적합 워커가 없을 때 — HR Lead 를 Task tool 로 호출해 자동 채용:",
            "```",
            "사유: 적합 워커 부재 (역할=<role>, 도메인=<domain>)",
            "행동:",
            "  1. dispatch tool 로 hr-lead 호출 (subagent_type=\"LSKunCompanyKit:hr-lead\")",
            "     → 채용 요청 (role, domain, 한 줄 사유)",
            ...
            "  4. 신규 워커에 Delegation Gate 판정 적용 → dispatch 또는 빙의 → 결재 → 사용자 응답",
            "```",
            "",
            "주의: 워커 → 워커 chain 은 금지. 워커는 CPO 에게만 보고하고",
            "다른 워커를 직접 호출하지 않는다. PreToolUse hook 이 차단한다.",
            "",
            "출력 위생 (ADR-0024): 도구 호출은 실제 tool call 로만 수행한다.",
            "`<invoke>` / `<function_calls>` / `Task(...)` 등 tool 구문을",
            "응답 텍스트로 출력하지 않는다. 위 응답 형식 블록도 개념 안내일 뿐",
            "그대로 재출력하지 말 것.",
            "",
            "Handoff Brief + Deep Work Protocol (ADR-0025 D5 + ADR-0024):",
            "dispatch prompt 에 CPO persona 의 Handoff Brief (목표/제약/관련 파일/",
            "기존 결정/기대 출력 형식/도구·소스 가이드/완료 기준 — CPO 직접 작성) 와",
            "작업 프로토콜 블록 (재해석·가정",
            "명시 → 근거·대안 → 검증 → 상세 보고) 을 JD 뒤에 주입한다.",
            "복잡·다단계·보안·아키텍처 작업은 모델 상속 유지 (강등 금지) +",
            "\"충분히 깊게 생각하고 진행하라\" 지시를 포함한다.",
            "",
