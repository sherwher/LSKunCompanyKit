...
5. HR Lead 를 `Task(subagent_type="LSKunCompanyKit:hr-lead", ...)` 로 dispatch — `templates/redteam.md`,
...
`subagent_type="LSKunCompanyKit:worker"`, ADR-0017 Allowlist + ADR-0026).
...
외주 dispatch 도 워커 dispatch 와 동일하게 `subagent_type="LSKunCompanyKit:worker"` 만 허용 (ADR-0026 D5 — 쓰기 도구 없음, 레드팀 destructive 금지와 같은 방향).
