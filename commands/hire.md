...
채용은 반드시 **파일 먼저 → audit 순서** 를 지킨다:

```
① adapter.create_worker(name, frontmatter_dict, body)   # 파일 먼저 — 실패 시 중단, audit 안 남김
② hire_audit.record_hire(actor=..., name=name, ...)     # 파일 성공 후에만 audit 기록
```

`create_worker` 가 실패(중복·권한 오류 등)하면 `record_hire` 를 호출하지 않는다.
파일 없는 audit 기록(고아 audit) = 유령참조 — ADR-0023 금지.
