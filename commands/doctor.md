...
- Local SSOT: `<project-root>/.company/` (P86 에서 `~/.lskun-companies/<name>/` 로 이전 예정)
- `company.md` 유무 / `hired/` 워커 수 / `archived/` 워커 수
- ~~dual-backend 감지~~ — ADR-0015 로 폐기 (Vault backend 자체가 폐기, 잔여 감지 불필요)
- 잔재 정리: `$LSKUN_VAULT` env var 가 설정되어 있으면 ⚠️ "ADR-0015 — `LSKUN_VAULT` env var 는 더 이상 plugin core 가 참조하지 않음. `/lskun-kit:sync-in --source=...` 명령의 인자로만 사용. 환경변수는 제거 권장."
...
> Local SSOT 가 아예 없는 (`.company/` 부재) 초기 상태는 8 / 9 / 10 / 11 / 13 / 14 / 15 / 16 모두 "(N/A — backend 없음)" 으로 skip. 17번은 plugin 자체 검사이므로 backend 무관하게 항상 수행.
...
[3]  Storage backend (Local 단일)   : ✅ .company/ 존재, workers=3, archived=0
