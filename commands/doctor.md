...
- 각 backend 별로 존재 여부 / `company.md` 유무 / `hired/` 워커 수
- **dual-backend 감지**: 양쪽 다 `company.md` 보유 시 ⚠️ "회사 자산이 한쪽에만 누적될 위험. `/lskun-kit:migrate` 로 동기화 권장. 자동 마이그레이션은 SSOT 정책상 금지." `lskun_kit.init.detect_dual_backend` 호출.
