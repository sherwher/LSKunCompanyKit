...
python3 "$LSKUN_CLI" --usage                 # audit log 기반 dispatch count + last_seen 컬럼 추가 (P109-A)
python3 "$LSKUN_CLI" --full --usage          # full + usage (워커별 사용 현황 진단)
```

> ADR-0019 (2026-05-27) — `--include-archived` 옵션 폐기. plugin core 는 archived/ 디렉토리를 더 이상 참조하지 않음.
>
> P109-A (2026-05-27) — `--usage` 옵션 신규. `.audit/decisions.jsonl` + 회전된 `decisions.<YYYY-MM>.jsonl.gz` (P109-B) 를 read-only 집계해 워커별 dispatch count + 가장 최근 dispatch 일자 표시. **사용자 명시 옵션** (자동 산출 X). 평가·점수·랭킹 0 — 단순 view (ADR-0002 §5 + ADR-0006 정신).
